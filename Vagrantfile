...
Vagrant.configure("2") do |config|
  config.vm.define "env" do |v|
    ...
    if has_proxy
      ...
      v.vm.provision :shell, :inline => "export http_proxy='#{proxy_url}' && /usr/bin/apt-get install -y puppet libaugeas-ruby augeas-tools rubygems lsb-release"
    else
      v.vm.provision :shell, :inline => "/bin/rm -rf /vagrant/volumes/mysql/*"
      v.vm.provision :shell, :inline => "/bin/sh /vagrant/docker/99_mysql_setup.sh"
      v.vm.provision :shell, :inline => "/usr/bin/apt-get update"
      v.vm.provision :shell, :inline => "/usr/bin/apt-get install -y puppet libaugeas-ruby augeas-tools rubygems lsb-release"
    end
